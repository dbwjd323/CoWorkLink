...
app.post("/join", function (req, res) {
  ...
  if (password !== passwordConfirm) {
    return res.redirect(
      "/join?error=비밀번호가 일치하지 않습니다. 다시 시도해주세요.&username=" +
        username +
        "&userID=" +
        userID +
        "&email=" +
  ...
  var sql =
    "INSERT INTO users (username, userID, password, passwordConfirm, email) VALUES (?, ?, ?, ?, ?)";
  client.query(
    sql,
    [username, userID, password, passwordConfirm, email],
...
app.post("/create", (req, res) => {
  ...
  const invitedUserIDsArray = Array.isArray(invitedUserIDs)
    ? invitedUserIDs
    : [invitedUserIDs];
  ...
  if (invitedUserIDsArray.length > 0) {
    ...
    client.query(
      checkUsersQuery,
      [invitedUserIDsArray],
      (checkUsersErr, checkUsersResults) => {
        if (checkUsersErr) {
        ...
        // 존재하지 않는 사용자 ID 필터링
        const nonExistingUserIDs = invitedUserIDsArray.filter(
          (userID) =>
            !checkUsersResults
        ...
        // 프로젝트 생성 로직 실행
        const projectQuery =
          "INSERT INTO projects (projectName, projectInfo, deadline, userID) VALUES (?, ?, ?, ?)";
        client.query(
          projectQuery,
          [projectName, projectInfo, deadline, req.session.userID],
          (projectErr, projectResult) => {
            ...
            const invitationsQuery =
              "INSERT INTO invitations (projectID, userID) VALUES ?";
            const values = invitedUserIDsArray.map((userID) => [
              projectId,
              userID,
            ]);
            ...
            client.query(invitationsQuery, [values], (invitationErr) => {
              ...
              const userProjectsValues = invitedUserIDsArray.map((userID) => [
                userID,
                projectId,
              ]);
              ...
              client.query(
                ...
                (userProjectsErr) => {
                  ...
                  console.log(data);
                  res.send({ success: true, message: alertMessage, projectId });
                },
              );
            });
          },
...
app.get("/projectPage", (req, res) => {
  if (!req.session.isLoggedIn) {
    ...
    client.query(
      `SELECT projectName, projectInfo, deadline FROM projects WHERE projectID = ?`,
      [projectID],
      (error, projectInfo) => {
        if (error) {
  ...
  client.query(
    ...
    (error, results) => {
      if (error) {
        ...
        res
          .status(500)
          .json({ error: "내부 서버 오류", message: error.message });
      } else {
        if (results.length === 0) {
...
app.get("/getTeammates", (req, res) => {
  const projectID = req.query.projectID;
  const getTeammatesQuery =
    "SELECT users.userID FROM users INNER JOIN invitations ON users.userID = invitations.userID WHERE invitations.projectID = ?";
...
app.post("/editProjectInfo", (req, res) => {
  ...
  const updateProjectQuery =
    "UPDATE projects SET projectName=?, projectInfo=?, deadline=? WHERE projectID=?";

  client.query(
    updateProjectQuery,
    [projectName, projectInfo, deadline, projectId],
    (error, results) => {
      if (error) {
...
app.delete("/deleteTeammate", (req, res) => {
  ...
  console.log(
    "Request to delete teammate. Project ID:",
    projectID,
    "Teammate ID:",
    teammateID,
  );

  const deleteTeammateQuery =
    "DELETE FROM invitations WHERE projectID = ? AND userID = ?";
...
app.get("/getUsersForTask", (req, res) => {
  const projectID = req.query.projectID;
  const getUsersForTaskQuery =
    "SELECT users.userID, users.username FROM users INNER JOIN invitations ON users.userID = invitations.userID WHERE invitations.projectID = ?";

  client.query(getUsersForTaskQuery, [projectID], (error, results) => {
    if (error) {
      console.error(
        "프로젝트에 초대된 사용자 목록을 가져오는 중 오류 발생:",
        error,
      );
      res.status(500).json({ error: "내부 서버 오류" });
    } else {
...
app.get("/getTasks", (req, res) => {
  ...
  if (!projectId) {
    return res
      .status(400)
      .json({ success: false, error: "프로젝트 ID가 제공되지 않았습니다." });
  }
...
app.post("/addTask", (req, res) => {
  ...
  const insertQuery =
    "INSERT INTO tasks (projectID, taskName, assignedTo, taskStatus) VALUES (?, ?, ?, ?)";
...
app.get("/getTaskDetails", (req, res) => {
  ...
  const taskDetailsQuery =
    "SELECT taskName, taskStatus, assignedTo FROM tasks WHERE taskID = ? AND projectID = ?";
  client.query(
    taskDetailsQuery,
    [taskID, projectID],
...
app.post("/saveTask", (req, res) => {
  ...
  const saveTaskQuery =
    "UPDATE tasks SET taskName=?, taskStatus=?, assignedTo=? WHERE  taskID = ? AND projectID = ?";
  client.query(
    saveTaskQuery,
    [taskName, taskStatus, assignedTo, taskID, projectID],
    (error, results) => {
      if (error) {
...
app.get("/myPageData", function (req, res) {
  if (!req.session.isLoggedIn) {
    ...
    const userID = req.session.userID;
    client.query(
      `SELECT username, password, email FROM users WHERE userID = ?`,
      [userID],
      (error, result) => {
        if (error) {
